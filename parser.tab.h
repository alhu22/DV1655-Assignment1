...
switch (yykind)
    ...
    // Implementation of make_symbol for each token kind.
      ...
      static
      symbol_type
      make_NULL (std::string v)
      {
        return symbol_type (token::NULL, std::move (v));
      }
#else
      static
      symbol_type
      make_NULL (const std::string& v)
      {
        return symbol_type (token::NULL, v);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
